...
const key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJ0Y2Z6b3FoemZ3dHppd3dnbmVzIiwicm9sZSI6ImFub24iLCJpYXQiOjE2ODk5MTI2ODcsImV4cCI6MjAwNTQ4ODY4N30.bBfWdDfLz8yKC48n8rhUd65MyEcm2eBafwLhSGkwf0E";

const url = "https://rtcfzoqhzfwtziwwgnes.supabase.co";
...
save.addEventListener("click", async (e) => {
    ...
    let res = await database.from("test1").insert({
        name: name,
        age: age,
        country: country
    })
...
const getStudent = async () => {
    ...
    const res = await database.from("test1").select("*");
    if (res) {
...
const getTotalCount = async () => {
    ...
    const res = await database.from("test1").select("*", { count: "exact" });
...
const editStudent = async (id) => {
    ...
    const res = await database.from("test1").select("*").eq("id", id);
    if (res) {
...
update.addEventListener("click", async () => {
    ...
    const res = await database.from("test1").update({
        name, age, country
    }).eq("id", id)
...
const deleteStudent = async (id) => {
    const res = await database.from("test1").delete().eq("id", id)
